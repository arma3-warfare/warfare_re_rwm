...
#define DEFAULT_FIELDREPAIR_EACH_PART_TIME 25
#define DEFAULT_FIELDREPAIR_EACH_HARDPART_TIME 45
...
// by Rubber: this repair only zlt_repair_hps array  this means that gun turret and instruments and other HIT part are not repaired, I tried to find the hitpart name of all but it may change by mod and mod so make life easier and repair all vehicle
/*zlt_fnc_partrepair = {
	private "_veh";
		...
		if (not isNil {_dmg}) then {
			if ( _dmg > 0.51 ) then {
				if (_x in zlt_hardRepairParts) then {
					_veh setHitPointDamage [_x,0.00];
				} else {
...
zlt_repair_hps = ["HitLFWheel","HitLBWheel","HitLMWheel","HitLF2Wheel","HitRFWheel","HitRBWheel","HitRMWheel","HitRF2Wheel" ,"HitEngine", "HitLTrack","HitRTrack","HitHull"] + ["HitFuel","HitAvionics","HitVRotor","HitHRotor"];
zlt_hardRepairParts = ["HitEngine", "HitLTrack","HitRTrack","HitHull"] + ["HitFuel","HitAvionics","HitHRotor"];
	...
	if (true) then {
		{
			...
			if (not isNil {_cdmg} ) then {
				if (_cdmg > 0.04) exitWith {_flag = true};
			};
		}  forEach zlt_repair_hps;
	};
	_flag;
...
zlt_frpr_getPartsRepairTime = {
	...
	_rprTime = 0;
	{
		...
		if (not isNil {_cdmg} ) then {
			...
			if (_cdmg > 0.51) exitWith {_rprTime = _rprTime + ( if (_x in zlt_hardRepairParts) then {DEFAULT_FIELDREPAIR_EACH_HARDPART_TIME} else {DEFAULT_FIELDREPAIR_EACH_PART_TIME}); };
		};
	}  forEach zlt_repair_hps;
	_rprTime;
};
...
if (isNil "zlt_cancelActionId") then {
	...
	player addAction["<t color='#ff0000'><img image='images\repair.paa'/> "+STR_FIELD_REPAIR+"</t>", zlt_prc_repairvehicle, [], -1, false, true, '','[] call zlt_fnc_repair_cond'];
