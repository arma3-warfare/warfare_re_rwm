...
while {alive _defense} do {
	if (isNull(gunner _defense) || !alive gunner _defense) then {
		...
		if (alive _closest && !(isNull _closest )) then {
			...
			// RUBBER: Markus set this value in strange way, according to http://community.bistudio.com/wiki/setSkill_array those value goes from 0 to 1
			_soldier setskill ["aimingAccuracy", 0.85];
			_soldier setskill ["aimingShake", 0.85];
			_soldier setskill ["aimingSpeed", 0.90];
			_soldier setskill ["endurance", 1000];
			_soldier setskill ["spotDistance", 0.99];
			_soldier setskill ["spotTime", 0.99];
			_soldier setskill ["courage", 0.99];
			_soldier setskill ["commanding", 0.99];
			_soldier setskill ["general", 0.99];
			/// CORE
			
			[str _side,'UnitsCreated',1] Call UpdateStatistics;
			
			diag_log Format["[WFRE (INFORMATION)][frameno:%3 | ticktime:%4] Server_HandleDefense: A Soldier has been dispatched to the %1 defense %2",str _side,_type,diag_frameno,diag_tickTime];
